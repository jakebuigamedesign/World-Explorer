...
const GEMINI_FALLBACK_MODELS = (process.env.GEMINI_FALLBACK_MODELS || 'gemini-2.0-flash,gemini-1.5-flash')
  ...
  .filter(Boolean);
const AI_PROVIDER = process.env.GEMINI_API_KEY ? 'gemini' : process.env.OPENAI_API_KEY ? 'openai' : 'offline';
...
function tutorInstructions() {
  return [
    ...
    'If the message is off-topic, rude, nonsense, random, role-play breaking, or unrelated to the lesson/scenario, do not engage with that content.',
    'For irrelevant messages, reply exactly: "This is not really relevant to this scenario. Please choose option A, B, or C, or ask about the words in this turn."',
...
function writingFeedbackInstructions() {
  return [
    ...
    'grammar: give 1-2 specific corrections from the student text, especially "is not allowed", "I do", spelling, verb forms, or punctuation.',
    'wordChoice: give one better phrase for an unnatural phrase from the student text and explain why it fits the situation better.',
...
function tutorGuardMessage(body) {
  ...
  if (/\b(asdf|qwer|blah|lorem ipsum|test test|random random|nothing|idk|i don't know|i dont know)\b/i.test(message) || /(.)\1{5,}/.test(message)) return TUTOR_REDIRECT;
...
function turnInstructions() {
  return [
    ...
    'A very smart or efficient choice may have positive time, such as time +5 or time +10, because staff help, a shortcut opens, a mistake is prevented, or the player avoids a delay.',
    'Do not make every good choice cost time. Every turn should usually include at least one choice with positive time.',
    'When the player gives a clear, prepared, efficient answer, prefer positive time unless there is a strong reason not to.',
    'At least one of the three choices should have positive time, especially the effective/polite option, unless all choices are genuinely bad.',
...
function normalizeTurn(parsed) {
  ...
  if (!normalizedChoices.some(choice => choice.time > 0)) {
    normalizedChoices[1].time = 8;
    normalizedChoices[1].stress = Math.min(normalizedChoices[1].stress, -6);
    normalizedChoices[1].reason = 'Efficient: this avoids a delay and saves time.';
  }
  return {
...
async function handleTurn(req, res) {
  if (!process.env.OPENAI_API_KEY) {
    send(res, 503, JSON.stringify({ error: 'Set OPENAI_API_KEY before starting server.js.' }));
    return;
  }
  ...
  const data = await apiRes.json().catch(() => ({}));
  if (!apiRes.ok) {
    send(res, apiRes.status, JSON.stringify({ error: data.error?.message || 'OpenAI API request failed.' }));
    return;
  }
