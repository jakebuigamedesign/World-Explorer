...
body{margin:0;min-height:100vh;padding:18px 12px;background:linear-gradient(145deg,#dfe8ff 0%,#f6f7fb 42%,#fff0df 100%);font-family:Arial,Helvetica,sans-serif;color:var(--ink)}
.app{max-width:860px;margin:auto;background:var(--bg);border-radius:22px;overflow:hidden;box-shadow:0 24px 60px rgba(34,47,95,.18)}
header{background:linear-gradient(135deg,var(--indigo),var(--indigo2));color:#fff;padding:18px 22px;display:flex;justify-content:space-between;gap:12px;align-items:center}
.brand{font-size:22px;font-weight:900}.sub{font-size:12px;opacity:.82;margin-top:2px}.pill{border:1px solid rgba(255,255,255,.4);background:rgba(255,255,255,.16);border-radius:999px;padding:7px 12px;font-size:13px;font-weight:800}
main{padding:24px}.screen{display:none}.screen.active{display:block}
...
.choice:hover{transform:translateY(-3px);border-color:var(--indigo);box-shadow:0 12px 26px rgba(57,73,171,.13)}
.emoji{font-size:38px;margin-bottom:9px}.name{font-weight:900;font-size:17px}.role{color:var(--indigo);font-size:13px;font-weight:800;margin:4px 0 8px}.desc{color:var(--muted);font-size:14px;line-height:1.55}
...
.btn{border:0;border-radius:13px;padding:12px 18px;font-size:14px;font-weight:900;cursor:pointer;font-family:inherit}.primary{background:linear-gradient(135deg,var(--indigo),var(--blue));color:#fff}.ghost{background:#fff;border:2px solid var(--line);color:var(--muted)}
.row{display:block;width:100%;text-align:left;background:#fff;border:2px solid var(--line);color:var(--ink);margin:7px 0;font-weight:800}.row:hover{border-color:var(--indigo);color:var(--indigo)}
...
.missionTop{display:flex;gap:14px;align-items:center;margin-bottom:13px}.missionIcon{font-size:46px}.tag{display:inline-block;background:var(--blueLight);color:var(--blue);border-radius:999px;padding:5px 10px;font-size:12px;font-weight:900}.stakes{background:var(--goldLight);border-left:5px solid var(--gold);padding:13px;border-radius:13px;line-height:1.55;margin:13px 0}.goal{background:var(--tealLight);border-left:5px solid var(--teal);padding:13px;border-radius:13px;line-height:1.55}
...
.word{background:#fff2d8;color:#8a4f00;font-weight:900;border-radius:6px;padding:1px 5px;border-bottom:2px solid #e7bd6d;cursor:pointer}
...
@media(max-width:700px){main{padding:18px}.grid,.instructionGrid{grid-template-columns:1fr}.meters{grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.meter{padding:8px}.meterLabel{font-size:11px;display:block}.meterLabel span{font-size:11px}.meterLabel span:last-child{display:block;margin-top:2px}.track{height:7px}header{align-items:flex-start;flex-direction:column}.pill{align-self:flex-start}.rules{font-size:12px}.miniHud{left:12px;right:12px;bottom:12px;transform:none;max-width:none}.introHero p{font-size:16px}.studyBar{gap:5px}.stopBtn{padding:7px 9px;font-size:12px}.questbar{font-size:12px;padding:8px 10px}.bubble{font-size:16px}}
</style>
</head>
<body>
<div class="toast" id="toast"></div>
<div class="app">
  <header><div><div class="brand">Word Explorer</div><div class="sub" id="apiStatus">Vocabulary role-play study · AI support available when server is running</div></div><div class="pill" id="hud">⭐ 0 XP</div></header>
  <main>
    <section class="screen active" id="start">
      <h1 class="title">Word Explorer</h1>
      <p class="subtitle">Use English to survive real-life challenges abroad. Protect your money, time, and calm.</p>
      <div class="card">
        <div class="note" style="margin-top:0"><b>Study mode</b><br>Try your best. This is a vocabulary learning game, not a school grade. Your choices, resources, and quiz answers will be saved for the research record.</div>
        <label><b>Participant ID or nickname</b></label>
        <input id="playerName" placeholder="Example: L01 or Jake">
        <label style="display:block;margin-top:12px"><b>Choose one difficulty tier</b></label>
        <div class="helperText" style="margin-top:4px">Click one option below. Easy gives more language support. Hard gives more challenge.</div>
        ...
        <button class="btn primary" style="width:100%;margin-top:12px" onclick="start()">Let's Go</button>
      </div>
    </section>
    <section class="screen" id="instructions">
      <button class="btn ghost" style="margin-bottom:12px;font-size:13px" onclick="show('start')">← Back</button>
      ...
      <div class="card">
        ...
        <div class="instructionGrid">
          ...
          <div class="infoTile data">
            <div class="infoIcon">📊</div>
            <div><div class="infoTitle">Study Data</div><div class="infoBody">Your choices, resources, quiz answers, writing response, and stop/finish point are saved automatically while the local server is running.</div></div>
          </div>
        </div>
        <div class="dataCallout">
          <b>Automatic collection:</b> when the game is opened through the server, events are saved to <b>study-data/word-explorer-events.jsonl</b>. The researcher page is available at <b>/research-data</b> on the same game link.
        </div>
...
function show(id){document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active'));$(id).classList.add('active');const hud=$('miniHud');if(hud)hud.classList.toggle('show',id==='game');updateSessionClock();updateXpSupportBar()}
...
function stopStudy(reason){
  ...
  $('studySummary').innerHTML=`<h3>Session summary</h3>
    ...
    <div class="note"><b>Data file:</b><br><span class="small">If you are using the local server, records are saved in <b>study-data/word-explorer-events.jsonl</b>.</span></div>
...
function start(){const n=$('playerName').value.trim();if(!n)return alert('Enter a participant ID or nickname first.');sound('choice');state.name=n;state.difficulty=($('difficultyTier')&&$('difficultyTier').value)||'Easy';logStudyEvent('session_start',{participantCode:n,difficulty:state.difficulty,studyAcknowledgement:true});show('instructions')}
...
function begin(){startSessionClock();resetLevelState();$('story').innerHTML='';$('dice').innerHTML='';$('responses').innerHTML='';positionMarkers();renderBars();logStudyEvent('level_start',{level:state.level,quest:state.level.quests[state.char.id],attemptNumber:state.attempts[state.level.id]});nextTurn();show('game')}
...
function renderTurnPage(){
  ...
  const label=p.title||((p.kind==='support')?'Support Page':(p.kind==='result'?'Result Page':`Turn ${p.turn} of ${TOTAL_TURNS}`));
  ...
  s.innerHTML=`<div class="pageNav">
      <button class="btn ghost" onclick="goStoryPage(-1)" ${previousDisabled}>← Back</button>
...
function onlineRequiredHtml(message){
  const detail=message?`<div class="small" style="margin-top:8px"><b>Current API message:</b> ${esc(message)}</div>`:'';
  return `<div class="note dangerNote"><b>AI API required.</b><br>This study version needs the local server to have a working Gemini or OpenAI API key with available quota.${detail}<div class="small" style="margin-top:8px"><b>Easiest setup:</b><br>1. Create a file named <b>.env</b> in the same folder as <b>server.js</b>.<br>2. Put either <b>GEMINI_API_KEY=your_key_here</b> or <b>OPENAI_API_KEY=your_key_here</b> inside it.<br>3. Restart the server with <b>node server.js</b><br>4. Open <b>http://127.0.0.1:8787/</b></div></div>`;
}
function showOnlineRequired(message){
  state.apiOk=false;
  state.aiChoices=[];
  $('apiStatus').textContent='AI support unavailable · check the local server/API key';
  $('dice').innerHTML='';
...
function sceneText(){
  ...
  const line={
    airport:[
      `Your bag is too heavy. You can pay an ${word('overweight fee')} or remove some items before we continue.`,
...
function freeChatGuardMessage(text){
  ...
  if(/\b(asdf|qwer|blah|lorem ipsum|test test|random random|nothing|idk|i don't know|i dont know)\b/i.test(raw)||/(.)\1{5,}/.test(raw))return freeChatRedirectMessage();
...
async function aiFeedbackForProduction(response,levelVocab,levelId){
  ...
  const fallbackNote='<div class="note dangerNote" style="margin-top:10px"><b>AI feedback unavailable.</b><br>The game used offline fallback feedback because the AI server or OpenAI API did not respond. For real data collection, check the Render deployment and API key/model.</div>';
  try{
